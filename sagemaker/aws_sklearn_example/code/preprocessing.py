...
def print_shape(df):
    ...
    print(
        "Data shape: {}, {} positive examples, {} negative examples".format(df.shape, positive_examples, negative_examples)
    )
...
if __name__ == "__main__":
    ...
    df.to_csv("census-income.csv", index=False)
    print(df.head())
    df.dropna(inplace=True)
    df.drop_duplicates(inplace=True)
    df.replace(class_labels, [0, 1], inplace=True)
    print(np.bincount(df["income"]))
    negative_examples, positive_examples = np.bincount(df["income"])
